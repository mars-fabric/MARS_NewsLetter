...
def top_companies_researcher_prompt(
    *,
    ...
    return _join([
        ...
        "",
        "## Procedure",
        f"1. **Bake the date range into every query.** Prepend or append `{date_from[:7]} OR {date_to[:7]}` (or `after:{date_from} before:{date_to}`) to each search so the engine biases toward in-window results.",
        "2. **Authority-first query order.** For each candidate company, run `site:<official-domain> news OR press OR release` as the FIRST query to confirm the domain and pull the canonical press/release page. The vendor page becomes the `official_domain` and the highest-priority evidence URL.",
        "3. Run each planned search via the DDGS / web-search tool.",
        f"4. **Strict date filter.** Ignore any hit whose URL year segment or snippet clearly points OUTSIDE `{date_from} → {date_to}` (e.g. `/2024/` in URL, snippet says 'in 2023'). Undated hits are acceptable for company-ranking evidence.",
...
def per_company_news_prompt(
    *,
    ...
    return _join([
        ...
        f"1. **First query — vendor-official (mandatory).** Run `site:<official-domain> 2025 OR 2026 announcement OR release OR launch OR news after:{date_from} before:{date_to}` via DDGS. The results from this query are the highest-authority hits and become `Primary source` (URL field) for any downstream story on that company.",
        f"2. **Second query — dated free-form.** Run `<company name> news {date_from[:7]}..{date_to[:7]}` (or `after:{date_from} before:{date_to}`) via DDGS. Use press-tier hits (reuters/bloomberg/ft/wsj/techcrunch) as `also_covered_by`.",
        f"3. **Third query — categorical.** Run `<company name> partnership OR funding OR product launch OR research {date_from[:4]}` via DDGS.",
        f"4. **Strict date filter.** Drop any hit whose URL year (e.g. `/2024/`, `/2023/`) or snippet timestamp is clearly OUTSIDE `{date_from} → {date_to}`. Undated hits: keep but tag `date: not stated in snippet`.",
...
def discovery_planner_prompt(
    *,
    ...
    return _join([
        ...
        "",
        f"**Bake the date range into every planned query.** Every query string MUST include either `{date_from[:7]}` / `{date_to[:7]}` (or `after:{date_from} before:{date_to}`) so the researcher's DDGS results are biased toward the window. Undated queries return stale content and are the #1 quality failure.",
        "",
        "**Authoritative-source bias (mandatory).** For every seed company from Stage 2-A and for every named vendor/regulator/product likely to surface, plan a dedicated `site:<official-domain>` query FIRST — the researcher will use that as `Primary source` and downgrade third-party press to `Also covered by`. Examples: `site:openai.com GPT release 2025 2026`, `site:anthropic.com Claude release 2025 2026`, `site:blogs.nvidia.com <topic> 2025 2026`, `site:sec.gov <topic> filing 2025`, `site:europa.eu AI Act 2025`.",
        "",
        ...
        "  - Search query (literal string, MUST include date qualifier)",
        "  - Why this query matters",
        ...
        f"Items whose snippet has no explicit date but whose URL year is `{date_from[:4]}` or `{date_to[:4]}` are acceptable — the curator will tag Freshness=6. Items whose URL/snippet clearly indicates a year outside the window will be dropped.",
    ])
...
def discovery_researcher_prompt(
    *,
    ...
    return _join([
        ...
        "",
        "## Procedure",
        f"1. **Bake the date range into every query.** Prepend or append the coverage window to each search string, e.g. `<query> {date_from[:7]} OR {date_to[:7]}` or `<query> after:{date_from} before:{date_to}`. Undated searches return stale content and are the #1 cause of out-of-window items in the final newsletter.",
        "2. **Authority-first query order.** For every seed company / vendor / regulator / product named in the plan, the FIRST query executed MUST be `site:<official-domain> <topic> <year>`. Only after that vendor-official query returns results (or is exhausted) do you run the generic press-search queries. The vendor page becomes `Primary source`; the third-party coverage becomes `Also covered by`.",
        "3. Execute each planned search via DDGS / web tools. From each result extract: title, URL, source domain, publication date if present, and a 3–5 sentence summary.",
        f"4. **Strict date filter (drop rule).** Drop any item whose visible date, URL year segment, or snippet text clearly indicates publication OUTSIDE `{date_from} → {date_to}`. Specifically: if the URL contains `/2024/`, `/2023/`, or any year not in `{date_from[:4]}` / `{date_to[:4]}`, DROP it. If the snippet says '2024' or '2023' as the story's own year, DROP it. If NO date signal is present (no snippet timestamp, no URL year), tag `date: not stated in snippet` and keep it — but flag `Freshness: 6` in the summary line so the curator downgrades it.",
        "5. Always include the URL with each item. Never invent URLs.",
        "6. **Vendor / regulator cross-check (mandatory follow-up).** When any item names a specific vendor (OpenAI/Anthropic/NVIDIA/Meta/Google), regulator (FDA/SEC/EU/FTC), or product (GPT-5/Claude/Gemini/Llama), run an additional `site:<official-domain>` search AS ITS OWN QUERY and add the canonical link as `Primary source`. The non-canonical hit becomes `Also covered by`. Do NOT skip this cross-check — it is what makes the newsletter trustworthy.",
        "7. If a search returns < 3 hits, simplify the query and retry once. If a `site:<official>` search returns 0 hits, that is acceptable — record a note in Coverage Notes.",
