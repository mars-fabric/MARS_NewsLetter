...
def curation_prompt(
    *,
    ...
    return _join([
        ...
        "4. **Drop only obvious noise.** Off-topic items, broken/placeholder URLs, items whose validation note reads exactly `dropped: unrelated`, and items with no real summary. **Do not** drop a user-provided URL for any other reason — `unknown` authority tier is fine.",
        "5. **User-Provided Links are first-class.** Every URL in the User-Provided Links block (validation table + enriched section) must appear at least once in your output unless it was flagged `dropped: unrelated`. Tag user-provided items with `Source: user-provided` on a dedicated line so the writer can route them into the right section.",
        "6. **Numeric scoring per item.** In addition to Category/Top tags, score each item on three 1–10 dimensions: `Relevance` (fit to industries / sub-domains / audience), `Authority` (source authority — official / authority / neutral / unknown), and `Freshness` (1 if outside the window, 10 if within and explicitly dated). These scores let Stage 4 rank inside each section.",
        "7. **Per-item summary** is 3–6 sentences, factual, no marketing language.",
        "8. **Tag categories.** Each item gets a `Category` (Product Launch / Research / Partnership / M&A / Funding / Regulatory / People / Trend / Other).",
        "9. **Highlight 'top stories'.** Tag the 5–8 highest-impact items with `Top: yes`. The writer will lead the newsletter with these. An item with `Top: yes` should also have Relevance ≥ 8 and Authority ≥ 7 except when no better source exists.",
        "",
        "## Output format (markdown)",
        ...
        "  - `## Curation Stats` — total items in raw, total kept, count of `Top: yes`, count of `Source: user-provided` items kept vs supplied, and a one-line gap analysis.",
        "",
...
def generation_writer_prompt(
    *,
    ...
    return _join([
        ...
        "- **User-Provided Links**: each URL in the `## User-supplied URLs` block above MUST appear inline as a citation at least once in the newsletter (typically inside `## 14. Source-Driven Insights → Insights from User-Provided Links`, but you may also use them earlier if the curated set tagged them as primary sources). Omit a user URL only when the curator labelled it `dropped: unrelated`.",
        "- **Ranking signals**: when the curated item has numeric tags (`Relevance`, `Authority`, `Freshness`, 1–10), pick items with the highest combined score for `Top Story of the Period`, `Secondary Major Story`, `Subdomain Highlights`, and `Focus Topic Deep Dive`. Within `Other Notable Headlines`, sort by `Top: yes` then by combined score.",
